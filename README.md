...
- **Configuration file**: runtime settings are read from `graph-config.json` in the directory the server or CLI starts from, or else from `$HOME/.graph-ops/config.json`. App Settings writes to that same file. Environment variables take precedence over any file, and a built-in default applies when nothing sets a value. The file also holds `projectPaths`, each project's local path in this environment; it is saved to whichever `graph-config.json` the server (or CLI) in use reads, so start them from places that resolve to the same file.
- **Four settings are read only from your home config**: `terminalCommand`, `claudeBinary`, `host` and `artifactsDir` come from an environment variable or `$HOME/.graph-ops/config.json`, and **never** from the `graph-config.json` of the directory you started in -- for those four, the home config is read even when a working-directory file exists. This is so that opening a repository you merely cloned cannot hand its author a command to run (`terminalCommand` and `claudeBinary` are executed when you press "Launch Claude"), publish this unauthenticated API to your network (`host`), or widen which files may be read back as artifacts (`artifactsDir`). If a working-directory `graph-config.json` does contain one of the four, startup prints a warning naming that file and those keys and then carries on -- nothing fails, and the values are simply not used. Saving any of the four from App Settings writes them to `$HOME/.graph-ops/config.json`, which is also what the page's "saved to" note then tells you. Since the warning is about the file, not the value, it keeps appearing until you delete those keys from that working-directory `graph-config.json` -- saving from App Settings writes the new value to your home config and deliberately leaves the repository's file alone. Every other setting keeps the behaviour described above.
  - **If you kept `artifactsDir` in a working-directory `graph-config.json`**, artifacts now go to the home config's value, or to `$HOME/.graph-ops/artifacts` if it does not set one -- so HTML and image artifacts that were registered from files under the old directory can no longer be read back. To keep using that directory, move the `artifactsDir` line into `$HOME/.graph-ops/config.json` (or set `GRAPH_ARTIFACTS_DIR`) before starting the UI again.
  - **If `$HOME/.graph-ops/config.json` itself cannot be read**, the CLI prints a warning naming that file and starts anyway, with these four falling back to environment variables and built-in defaults. App Settings shows the same warning, but saving "Artifacts directory" fails there with a message naming the file: it will not overwrite a config file it could not parse. Repair or delete that file, then save again.
...
- **設定ファイル**: 実行時の設定は、サーバーや CLI を起動したディレクトリの `graph-config.json`、なければ `$HOME/.graph-ops/config.json` から読み込まれます。アプリ設定もこのファイルに書き込みます。環境変数はどのファイルよりも優先され、どこにも値がなければ組み込みの既定値が使われます。このファイルには、この環境での各プロジェクトのローカルパス `projectPaths` も入ります。使っているサーバー（または CLI）が読む `graph-config.json` に保存されるので、両者が同じファイルを読む場所から起動してください。
- **4 つの設定はホームの設定ファイルからしか読まれません**: `terminalCommand`・`claudeBinary`・`host`・`artifactsDir` は、環境変数か `$HOME/.graph-ops/config.json` からのみ読み込まれ、起動したディレクトリの `graph-config.json` からは**読み込まれません**。この 4 つについては、作業ディレクトリに設定ファイルがあってもホームの設定が読まれます。取得しただけのリポジトリを開いて起動しただけで、同梱の設定にコマンドを実行させたり（`terminalCommand` と `claudeBinary` は「Claude を起動」で実行されます）、認証のないこの API をネットワークへ公開したり（`host`）、成果物として読み出せるファイルの範囲を広げたり（`artifactsDir`）できないようにするためです。作業ディレクトリの `graph-config.json` にこの 4 つが書かれていた場合は、起動時にファイル名とキー名を挙げた警告が出るだけで、起動は失敗しません（値が使われないだけです）。アプリ設定からこの 4 つを保存すると `$HOME/.graph-ops/config.json` に書き込まれ、画面の「保存先」の表示もそのパスになります。警告は値ではなくファイルの内容に対して出るので、止めるには作業ディレクトリの `graph-config.json` から該当のキーを削除してください（アプリ設定から保存しても、新しい値はホームの設定ファイルに書かれるだけで、リポジトリ側のファイルはあえて書き換えません）。それ以外の設定は、上に書いたとおりの挙動のままです。
  - **作業ディレクトリの `graph-config.json` に `artifactsDir` を置いていた場合**、成果物の保存先はホームの設定の値（設定がなければ `$HOME/.graph-ops/artifacts`）に変わります。そのため、以前のディレクトリ配下のファイルから登録した HTML・画像の成果物は読み出せなくなります。同じディレクトリを使い続けるには、UI を起動し直す前に `artifactsDir` の行を `$HOME/.graph-ops/config.json` へ移してください（または環境変数 `GRAPH_ARTIFACTS_DIR` を設定してください）。
  - **`$HOME/.graph-ops/config.json` 自体が読み込めない場合**、CLI はそのファイル名を挙げた警告を出したうえで起動を続け、この 4 つは環境変数と組み込みの既定値にフォールバックします。アプリ設定にも同じ警告が出ますが、「成果物ディレクトリ」の保存はファイル名を挙げたエラーになります（読み込めなかった設定ファイルを上書きしないためです）。このファイルを直すか削除してから保存し直してください。
