...
// immediately PATCHes the ticket's complete label set; the checkboxes are
// disabled while that request is in flight. Clicks never reach the ticket
...
export const LabelSelect: React.FC<Props> = ({ ticketId, labels, projectLabels, onSaved }) => {
  ...
  const toggle = async (id: string) => {
    ...
    const next = selectedIds.includes(id)
      ...
      : projectLabels.map(l => l.id).filter(x => x === id || selectedIds.includes(x));
    setSaving(true);
    setError('');
    try {
              ...
              projectLabels.map(l => (
                ...
                >
                  <input
                    ...
                    className="rounded border-slate-300 dark:border-slate-600 text-blue-600 focus:ring-0 disabled:opacity-50"
                  />
