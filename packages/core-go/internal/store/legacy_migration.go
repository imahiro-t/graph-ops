...
// (MEDIUM). The data is normalized here, once, rather than by substituting
// MEDIUM on every read.
//
// It is idempotent and safe under concurrent Init calls: the WHERE clause
// only ever matches NULL rows, so a second (or racing) run updates nothing
...
func backfillNullTicketPriority(db *sql.DB) error {
	res, err := db.Exec(`UPDATE tickets SET priority = ? WHERE priority IS NULL`, string(domain.DefaultTicketPriority))
	if err != nil {
		return fmt.Errorf("backfilling NULL ticket priority: %w", err)
	}
