...
//  1. an explicit project_id in the body;
//  2. the project the given ticketId belongs to;
//  3. the server's global TerminalWorkDir fallback (unchanged behavior for
//     callers that don't know about projects at all).
//
// Any lookup failure along the way (unknown id, ticket without a project,
// etc.) is treated as "fall through to the next priority", not an error --
...
func (s *Server) resolveLaunchWorkDir(projectID, ticketID string) string {
	...
	if ticketID != "" {
		if t, err := s.repo.GetTicket(ticketID); err == nil && t != nil && t.ProjectID != "" {
			if p, err := s.repo.GetProject(t.ProjectID); err == nil && p != nil {
				return p.WorkDir
			}
		}
	}
