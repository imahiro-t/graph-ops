...
//
// The answer lives in graph-config.json's currentProjectId -- a
// per-environment file -- not in the data source. Before DFLT-00106 it was
...
// errAlreadyInherited aborts Get's inheritance write without rewriting
// graph-config.json when another writer set currentProjectId between Get's
// read and its update (runtimeconfig.Update re-loads the file under its
...
//
// When graph-config.json has no currentProjectId at all (the key absent,
...
// would create a graph-config.json on a brand-new install just because
...
func Get(cwd, home string, repo store.GraphRepository, logger *slog.Logger) (string, error) {
	...
	saved, path, err := runtimeconfig.Update(cwd, home, func(c *runtimeconfig.FileConfig) error {
	...
	case err != nil:
		log(logger).Warn("failed to save the current project inherited from the data source to graph-config.json; it will be inherited again on the next read",
...
// graph-config.json -- the DB's app_state is never written any more, so
...
// anything is stored.
func Set(cwd, home, projectID string) error {
	_, _, err := runtimeconfig.Update(cwd, home, func(cfg *runtimeconfig.FileConfig) error {
		SetIn(cfg, projectID)
		return nil
	})
	return err
...
func Clear(cwd, home string) error {
	return Set(cwd, home, "")
}

// SetIn applies projectID to an already-loaded FileConfig, for a caller that
// is inside a runtimeconfig.Update of its own and must change
