...
2. Fetch the fixed template by running `get-review-template`.
...
4. Judge pass or fail against those criteria, and fill in the template's
   four headings -- 判定, 指摘事項, 判断理由, 条件付き承認の場合の条件, in that order --
   with real content in place of the REPLACE_WITH_* placeholders. Do not
   delete, reorder, rename, or add headings.
