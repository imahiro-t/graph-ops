...
1. Call `get-executable` to get the runnable nodes.
   ...
   - If `get-executable` itself fails (exits non-zero with an error), simply call it again: a failed call hands out nothing and puts back every node it had claimed, so the retry sees them as runnable again. Only if the error says it failed to release claimed node(s) and names them, run `graph-engine unstick-node "<nodeId>"` on each named node before retrying -- those are left `IN PROGRESS`/`IN REVIEW` with no subagent running them.
