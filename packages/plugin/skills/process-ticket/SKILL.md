...
```
This returns `{"resolved": "ja"|"", "source": "user"|"none", "supported_locales": ["ja"]}`. The working language is a personal setting: only the user tier is consulted, and a `language` in the team tier's `workflow.yaml` is ignored.
- If `source` is `"user"`, a persistent setting already exists -- **do not** pass `--language` to `get-executable`/`expand-graph` below; let them resolve it themselves from the user tier, so this session never silently overrides a deliberately-configured language just because of how this one conversation happens to be phrased. (If the user explicitly asks, in this session, to use a different language for this ticket only, that request overrides this default -- pass `--language <code>` for that one ticket's calls in that case.)
...
1. Call `get-executable` to get the runnable nodes.
   ...
   - If `blocked: true` and no manual node is waiting on a first decision, before reporting anything to the user look at *why* it is blocked in `get-ticket`: an `approval_gate` at `REJECTED` means a rejection to triage (step 4), while a review/review_gate still at `IN REVIEW` next to a loop target whose `iteration_count + 1` has reached its `max_iterations` (the review failed in its last allowed round) means a loop ran out of retries (step 5) -- that loop target is usually still `DONE`, but it can also be sitting at `TODO`, which is recoverable the same way. The two have different entry points and different recoveries.
...
**One round of rework costs one iteration, however many gates sent it back.** The loop target's `iteration_count` counts how many times *it* was redone, so several gates hanging off the same target and rejecting in the same round spend one iteration between them, not one each. Hanging four review gates off `impl` therefore does not mean the workflow-wide review iteration limit (`max_iterations`, 3 by default) is used up by a single round of rework, and the second and later rejections of a round write nothing to the loop target and cannot block the ticket. Two edges of that rule are worth knowing:
...
`max_iterations` is a limit on **review rounds**, counting the first review: it is one workflow-wide setting (3, 4 or 5; 3 by default) stored on every node when the graph is built, so a later settings change does not affect a ticket already in progress. (The seed nodes store the value in effect when the seed is created, the rest of the graph the value in effect at `expand-graph`, so a change in between leaves `plan`/`plan_review` and the later nodes with different limits.) The round being judged is the loop target's `iteration_count + 1`, and `get-review-criteria` reports it as `Review round: <r> / <limit>` together with the convergence tier it is judged at (3: Normal / Important / Final; 4: Normal x2 / Important / Final; 5: Normal x2 / Important x2 / Final).

This is a different entry point from step 4, with a different cause: nothing was rejected by a human and there is no `rejection_reason` to read. A review/review_gate failed in the last allowed round -- its loop target at `iteration_count == max_iterations - 1` -- so the engine blocked the ticket instead of looping back once more. With a limit of 3 the reviews ran rounds 1, 2 and 3, and the target's `iteration_count` stays at 2. `get-ticket` shows the ticket `blocked: true`, the loop target at its last round, and the failing reviewer still `IN REVIEW` (deliberately: a blocked loop-back writes nothing at all, so there is no half-rewound graph to reason about). The loop target is usually `DONE`, but it can be `TODO` instead -- an earlier rejection had already rewound it, or the ticket was blocked by an older release that spent one iteration per failing gate rather than one per round. Both cases recover the same way.

`reopen-nodes` alone is not the recovery. It clears the blocked flag but leaves `max_iterations` where it is, so the very next rejection blocks the ticket again; and when the loop target is `DONE` it refuses the call outright, because reopening a finished loop target starts a new round and it refuses the whole call when that round would pass the target's `max_iterations` -- which it does by definition here. The recovery is two commands, plus a third only if something is still claimed:
