...
When the user says the new ticket is derived from an existing ticket (a follow-up, a split-off part), pass `--parent <ticketId>` to make it that ticket's child. With `--parent` and no `--project`, the ticket goes into the parent's project and nothing is printed to stderr; an unknown parent is `TICKET_NOT_FOUND`, a `--project` other than the parent's is `VALIDATION_ERROR`, and an HTTP data source still on protocol 1.0 answers `PARENT_TICKET_UNSUPPORTED` -- in each case no ticket is created. The parent is set at creation only and cannot be changed later; `graph-engine get-ticket` shows it as `parent` (and the parent's `children`). `/graph-ops:autopilot-tree` processes a ticket's children after the ticket itself.
