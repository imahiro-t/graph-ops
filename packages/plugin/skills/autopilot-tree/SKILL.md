...
When the line has `untrusted_folder` (with or without `--run`), Claude Code has not trusted that folder yet, so the sessions the run opens there stop at the workspace trust dialog. Tell the person in one line to open that folder in Claude Code once and accept the trust prompt (or accept it in a terminal already waiting at the prompt), then carry on without stopping or asking. The field is a best-effort notice; its absence does not prove the folder is trusted.
...
- `launch`: run `command`. It opens a child session in another terminal and prints the `wait` command to run next. When its line has `untrusted_folder`, tell the person in one line that the new terminal is likely waiting at the workspace trust prompt for that folder and needs them to accept it, then carry on to `wait` as usual. A `launch` with `--role merge-up` (a merge that could not fast-forward) or `--role finalize` (the finalize action: the reflection into the main branch once the whole tree is done) is run the same way.
