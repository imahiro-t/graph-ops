...
- An `osascript` normally finishes in a few seconds, so even with several launches waiting the lock comes free for each of them well within 15 seconds.
- The wait runs out only when a holder's `osascript` runs close to its 10-second limit, which happens when something is wrong (a permission prompt left unanswered, for example). The waiting launch's own tab would then most likely fail for the same reason.
