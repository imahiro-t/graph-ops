...
| No new tab appeared within about 3 seconds of Cmd+T, it closed before the command could be run in it, or it opened in another Terminal window than the orchestrator's (the Cmd+T reached another window). The command is not run in it. | This launch opens a new window; the next launch tries again. A tab that opens late, or in another window, is left empty; `graph-engine` does not close it, so close it yourself. |
