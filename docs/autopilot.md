...
| No new tab appeared within about 3 seconds of Cmd+T, it closed before the command could be run in it, or it opened in another Terminal window than the orchestrator's (the Cmd+T reached another window). The command is not run in it. | This launch opens a new window; the next launch tries again. A tab that opens late, or in another window, is left empty; `graph-engine` does not close it, so close it yourself. It does not, because a late tab may appear only after the tab script has finished, and Terminal's scripting has no command that closes a single tab: typing `exit` into it does not close it under every profile and would mean typing into a tab outside the orchestrator's window, and closing a window could close one of yours. The empty tab runs nothing but an idle shell. |
