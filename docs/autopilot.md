...
**When a tab is not used, the session opens in a new window** (the same `open -a Terminal <script>.command` as before), and the launch itself does not fail on the tab's account; only when the new window cannot be opened either does `launch` fail as before. Each fallback prints one warning to the orchestrator's stderr, `graph-engine: warning: the <role> session of <ticket> opened in a new Terminal window because a tab could not be opened (<reason>)`, and the one-line JSON on stdout is unchanged. There are two kinds of fallback:
...
| No new tab appeared in the window within about 3 seconds of Cmd+T, or it closed before the command could be run in it. | This launch opens a new window; the next launch tries again. |
| More than one new tab appeared at once (another run, or you, opened a tab in the same window at the same moment), so the new tab cannot be told apart. The command is run in none of them. | This launch opens a new window; the next launch tries again. |
| Another `graph-engine` process was opening a Terminal tab and did not finish within 10 seconds (see the lock below). | This launch opens a new window without trying; the next launch tries again. |
| Anything else: a missing Accessibility or Automation permission, a sandbox that refuses Apple events, `osascript` missing or failing in any other way, or `osascript` not finishing within 10 seconds (for example while a permission prompt is left unanswered). | This launch opens a new window, and **the tab is disabled for the rest of the run**: later sessions go straight to new windows, and the warning says so. |
...
**The per-run disable.** When a tab is disabled, the reason (up to 300 characters) is kept in the run file as `terminal_tab_disabled`, so the rest of the run does not wait for the same failure on every launch. It is cleared, and the tab tried again, when the run is taken over (running the same command again after an interruption) or adopted by a new orchestrator. So after granting a missing permission, interrupting the orchestrator and running the same command again brings the tabs back for that run. A `launch` that failed altogether (not even a new window opened) never disables the tab.

**One tab at a time across processes.** Tab launches are serialized through a lock on `$HOME/.graph-ops/autopilot/terminal-tab.lock` (an advisory `flock`, released when the process ends, so a crash never leaves it held), held only while `osascript` runs. Two runs whose orchestrators share a window therefore do not open their tabs at the same moment; a launch that waits more than 10 seconds for the lock falls back as in the table. If the lock file cannot be created, the tab is tried without the lock -- identifying the new tab by its tty, and refusing when there is more than one, is what keeps a command out of the wrong tab.

**How long a launch can take.** In the worst case, `launch` waits up to 10 seconds for the lock, up to 10 seconds for `osascript`, and up to 10 seconds for `open -a Terminal`, about 30 seconds in all. This happens outside the run registry's lock, so other commands are not held up.
...
| `launch <runId> <ticketId> [--role work\|merge-up\|finalize]` | Prepares the worktree and opens the child session. `{"launched","role","worktree","branch","base_branch","next"}`; `next` is the `wait` command. A failure prints the error with `attempt n of 2`; the second failure in a row records the ticket as failed (`launch_failed`). In Terminal.app, a session that opened in a new window instead of a tab also prints a `graph-engine: warning:` line on stderr (see [Terminal.app: child sessions as tabs](#terminalapp-child-sessions-as-tabs)). |
