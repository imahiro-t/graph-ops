...
- **Web UI**: ローカルの UI サーバーは既定で `127.0.0.1` だけで待ち受け、UI 自体は外部サイトからリソースを読み込みません。設定の `host`（または `GRAPH_HOST`）を変えない限り、他のマシンからは接続できません。ただし、UI でプレビューする HTML 成果物は、その成果物自体が参照している外部リソースを読み込むことがあります。
